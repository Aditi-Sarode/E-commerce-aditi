...
export default function Product() {
  ...
  const handleCheckboxChange = (value, sectionId) => {
    const updatedValue = selectedFilters[sectionId] === value ? '' : value
    setSelectedFilters({ ...selectedFilters, [sectionId]: updatedValue })
    updateQueryString(sectionId, updatedValue)
  }

  // Handle radio filter selection
  const handleRadioChange = (e, sectionId) => {
    const value = e.target.value
    setSelectedFilters({ ...selectedFilters, [sectionId]: value })
    updateQueryString(sectionId, value)
  }
          ...
          <div className="fixed inset-0 z-40 flex">
            ...
            >
              ...
              <form className="mt-4 border-t border-gray-200">
                ...
                {/* Display other filters (checkboxes for color, size, etc.) */}
                {filters.map((section) => (
                ...
                {/* Radio button for Price, Discount Range, and Availability */}
          ...
          <section aria-labelledby="products-heading" className="pb-24 pt-6">
            ...
            <div className="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-5">
              ...
              <form className="hidden lg:block">
                {/* Render filters like color, size */}
                {filters.map((section) => (
                ...
                {/* Radio buttons for price, discount range, availability */}
                {singleFilter.map((section) => (
                  <Disclosure key={section.id} as="div" className="border-b border-gray-200 py-6">
                    <h3 className="-my-3 flow-root">
                      <DisclosureButton className="group flex w-full items-center justify-between bg-white py-3 text-sm text-gray-400 hover:text-gray-500">
                ...
                {singleFilter.map((section) => (
                  <Disclosure key={section.id} as="div" className="border-t border-gray-200 py-6">
                    ...
                    <DisclosurePanel className="pt-6">
                      <div className="space-y-4">
                        <FormControl component="fieldset">
                          <RadioGroup aria-label={section.name} name={`${section.id}-group`}>
                            {section.options.map((option) => (
                              <FormControlLabel
                                key={option.value}
                                value={option.value}
